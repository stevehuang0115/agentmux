...
import {
  ...
  updateEntityPositionMapInPlace,
...
describe('Factory Collision', () => {
  ...
  describe('updateEntityPositionMapInPlace', () => {
    it('should update an empty map with new entities', () => {
      const existing = new Map<string, { x: number; z: number }>();
      const agents = new Map<
        string,
        { currentPosition?: { x: number; z: number }; basePosition: { x: number; z: number } }
      >();
      agents.set('agent-1', { currentPosition: { x: 5, z: 10 }, basePosition: { x: 0, z: 0 } });
      const npcs = new Map<string, { x: number; z: number }>();
      npcs.set('npc-1', { x: 15, z: 20 });

      updateEntityPositionMapInPlace(existing, agents, npcs);

      expect(existing.size).toBe(2);
      expect(existing.get('agent-1')).toEqual({ x: 5, z: 10 });
      expect(existing.get('npc-1')).toEqual({ x: 15, z: 20 });
    });

    it('should update existing positions in-place when changed', () => {
      const existingPos = { x: 0, z: 0 };
      const existing = new Map<string, { x: number; z: number }>();
      existing.set('agent-1', existingPos);
      ...
      updateEntityPositionMapInPlace(existing, agents, new Map());

      // Should be the same object reference, updated in-place
      expect(existing.get('agent-1')).toBe(existingPos);
      expect(existingPos.x).toBe(5);
      expect(existingPos.z).toBe(10);
    });

    it('should not modify position object when values are unchanged', () => {
      const existingPos = { x: 5, z: 10 };
      const existing = new Map<string, { x: number; z: number }>();
      existing.set('agent-1', existingPos);
      ...
      Object.freeze(existingPos);

      // Should not throw - no mutation occurs when values unchanged
      expect(() => updateEntityPositionMapInPlace(existing, agents, new Map())).not.toThrow();
    });

    it('should remove deleted entities', () => {
      const existing = new Map<string, { x: number; z: number }>();
      existing.set('agent-1', { x: 0, z: 0 });
      existing.set('agent-2', { x: 5, z: 5 });
      existing.set('npc-1', { x: 10, z: 10 });

      const agents = new Map<
        string,
        { currentPosition?: { x: number; z: number }; basePosition: { x: number; z: number } }
      >();
      agents.set('agent-1', { currentPosition: { x: 0, z: 0 }, basePosition: { x: 0, z: 0 } });
      // agent-2 is removed
      ...
      expect(existing.has('agent-1')).toBe(true);
      expect(existing.has('agent-2')).toBe(false);
      expect(existing.has('npc-1')).toBe(false);
    });

    it('should add new entities while keeping existing ones', () => {
      const existing = new Map<string, { x: number; z: number }>();
      existing.set('agent-1', { x: 0, z: 0 });
      ...
      >();
      agents.set('agent-1', { currentPosition: { x: 0, z: 0 }, basePosition: { x: 0, z: 0 } });
      agents.set('agent-2', { currentPosition: { x: 5, z: 5 }, basePosition: { x: 0, z: 0 } });

      const npcs = new Map<string, { x: number; z: number }>();
      npcs.set('npc-1', { x: 10, z: 10 });

      updateEntityPositionMapInPlace(existing, agents, npcs);

      expect(existing.size).toBe(3);
      expect(existing.get('agent-1')).toEqual({ x: 0, z: 0 });
      expect(existing.get('agent-2')).toEqual({ x: 5, z: 5 });
      expect(existing.get('npc-1')).toEqual({ x: 10, z: 10 });
    });

    it('should use basePosition when currentPosition is undefined', () => {
      const existing = new Map<string, { x: number; z: number }>();
      const agents = new Map<
        string,
        { currentPosition?: { x: number; z: number }; basePosition: { x: number; z: number } }
      >();
      agents.set('agent-1', { currentPosition: undefined, basePosition: { x: 7, z: 14 } });

      updateEntityPositionMapInPlace(existing, agents, new Map());

      expect(existing.get('agent-1')).toEqual({ x: 7, z: 14 });
    });

    it('should handle NPC overriding agent with same ID', () => {
      const existing = new Map<string, { x: number; z: number }>();
      ...
      const agents = new Map<
        ...
        { currentPosition?: { x: number; z: number }; basePosition: { x: number; z: number } }
      >();
      agents.set('shared-id', { currentPosition: { x: 1, z: 1 }, basePosition: { x: 0, z: 0 } });
      ...
      // NPC is processed after agent, so NPC value should win
      expect(existing.get('shared-id')).toEqual({ x: 99, z: 99 });
    });

    it('should handle all empty inputs', () => {
      const existing = new Map<string, { x: number; z: number }>();
      existing.set('old-entity', { x: 1, z: 1 });

      updateEntityPositionMapInPlace(existing, new Map(), new Map());
