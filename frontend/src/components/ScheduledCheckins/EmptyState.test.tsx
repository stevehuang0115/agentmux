...
describe('EmptyState', () => {
  ...
  it('renders logs empty state correctly', () => {
    ...
    expect(screen.queryByText('No active messages')).not.toBeInTheDocument();
    expect(screen.queryByText('No completed messages')).not.toBeInTheDocument();
  });
  ...
  it('renders icon for active state', () => {
    const { container } = render(<EmptyState type="active" onCreateMessage={vi.fn()} />);

    const svg = container.querySelector('svg');
    expect(svg).toBeInTheDocument();
  });

  it('renders icon for completed state', () => {
