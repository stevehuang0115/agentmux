/**
 * TigerAgent - Animated tiger character model.
 *
 * Loads the tiger GLB model with Mixamo animations and positions it based on agent status.
...
export const TigerAgent: React.FC<TigerAgentProps> = ({ agent }) => {
  ...
  // Load tiger model with animations
  const gltf = useGLTF(MODEL_PATHS.TIGER);
  ...
  const { currentAnimation } = useAgentAnimation(agent, actions, AGENT_ANIMATION_CONFIGS.TIGER);
  ...
  useFrame((state, delta) => {
    ...
    if (isIdle) {
      // Stage performance - go dance on stage (Tiger does 'Salsa dancing')
      if (idleActivity === 'stage' && isOnStage) {
        ...
        groupRef.current.position.y = distance < 0.5 ? FACTORY_CONSTANTS.STAGE.HEIGHT + TIGER_Y_OFFSET : TIGER_Y_OFFSET;

        if (distance < 0.5 && actions?.['Salsa dancing']) {
          if (walkState.currentAnim !== 'Salsa dancing') {
            Object.values(actions).forEach(action => action?.fadeOut(0.3));
            actions['Salsa dancing']?.reset().fadeIn(0.3).play();
            walkState.currentAnim = 'Salsa dancing';
          }
        }
        return;
      }
      ...
      if (idleActivity === 'couch' && couchIndex >= 0 && couchPositions[couchIndex]) {
        ...
        groupRef.current.position.y = TIGER_Y_OFFSET;
    ...
    // Normal behavior - Position the tiger at workstation
    if (isActuallyWorking) {
      ...
      // Tiger has larger wander area since it runs
      const wanderRadiusX = isCoffeeBreak ? 3.0 : 4.0;
      const wanderMinZ = workstation.position.z + 2.5;
      const wanderMaxZ = workstation.position.z + 6.0;
      ...
      // Determine if we should be moving (have a target to reach)
      const shouldBeMoving = distance > 0.3;

      if (shouldBeMoving) {
        // Use Running for longer distances, Walking for shorter (tiger prefers running)
        const shouldRun = distance > 1.5;
        const speed = shouldRun ? 3.5 : 1.5;
        const animName = shouldRun ? 'Running' : 'Walking';
        const moveAction = actions?.[animName] || actions?.['Walking'];
  ...
  return (
    <group
      ref={groupRef}
      position={[agent.basePosition.x, TIGER_Y_OFFSET, agent.basePosition.z]}
      rotation={[0, Math.PI, 0]}
    >
      {/* Blue circle indicator under real agent - positioned at ground level */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -TIGER_Y_OFFSET + 0.1, 0]}>
