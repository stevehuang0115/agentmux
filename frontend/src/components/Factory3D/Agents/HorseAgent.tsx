/**
 * HorseAgent - Animated horse character model.
 *
 * Loads the horse GLB model with Mixamo animations and positions it based on agent status.
...
export const HorseAgent: React.FC<HorseAgentProps> = ({ agent }) => {
  ...
  // Load horse model with animations
  const gltf = useGLTF(MODEL_PATHS.HORSE);
  ...
  const { currentAnimation } = useAgentAnimation(agent, actions, AGENT_ANIMATION_CONFIGS.HORSE);
  ...
  useFrame((state, delta) => {
    ...
    if (isIdle) {
      // Stage performance - go perform on stage (Horse does 'Talking')
      if (idleActivity === 'stage' && isOnStage) {
        ...
        groupRef.current.position.y = distance < 0.5 ? FACTORY_CONSTANTS.STAGE.HEIGHT + HORSE_Y_OFFSET : HORSE_Y_OFFSET;

        if (distance < 0.5 && actions?.['Talking']) {
          const performAction = actions['Talking'];
      ...
      if (idleActivity === 'couch' && couchIndex >= 0 && couchPositions[couchIndex]) {
        ...
        groupRef.current.position.y = HORSE_Y_OFFSET;
    ...
    // Normal behavior - Position the horse at workstation
  ...
  return (
    <group
      ref={groupRef}
      position={[agent.basePosition.x, HORSE_Y_OFFSET, agent.basePosition.z]}
      rotation={[0, Math.PI, 0]}
    >
      {/* Blue circle indicator under real agent - positioned at ground level */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -HORSE_Y_OFFSET + 0.1, 0]}>
