...
import { FileStorage, Project, Team, Assignment, ActivityEntry } from '../services/FileStorage.js';
...
export class AgentMuxMCPServer {
  ...
  private async handleGetProjectDetails(args: any) {
  ...
  private async handleListTeams(args: any) {
  ...
  private async handleAssignTeamToProject(args: any) {
  ...
  private async handleGetActivityStatus(args: any) {
    const { teamId, projectId } = args;
  ...
  private async handleGetActivityTimeline(args: any) {
  ...
  private async handleListTmuxSessions(args: any) {
  ...
  private async handleCaptureSessionOutput(args: any) {
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError, 
        `Failed to capture session output: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  // Control Handlers
  private async handlePauseTeam(args: any) {
    const { teamId } = args;
  ...
  private async handleResumeTeam(args: any) {
    const { teamId } = args;
  ...
  private async handleEndAssignment(args: any) {
    ...
    if (!updatedAssignment) {
      throw new McpError(ErrorCode.InvalidRequest, `Assignment not found: ${assignmentId}`);
    }
