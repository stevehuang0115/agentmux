...
import { AuthService, validateAuthInput, AuthenticatedRequest } from '../middleware/auth';

export function createAuthRoutes(userStore: UserStore, authService: AuthService): Router {
  const router = Router();

  // Register new user
  router.post('/register', validateAuthInput.register, async (req: Request, res: Response) => {
    try {
  ...
  // Login user
  router.post('/login', validateAuthInput.login, async (req: Request, res: Response) => {
    try {
  ...
  router.post('/change-password', authService.authenticate(), async (req: AuthenticatedRequest, res: Response) => {
    try {
