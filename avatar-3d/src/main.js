...
function createRobotWithAnimalHead(x, z, animalType = 'cow') {
  ...
  // Clone the robot model properly
  const robot = robotModelTemplate.clone();
...
function createNameCard(text, x, y, z) {
  ...
  card.position.y = 0.02; // Just above floor
  group.add(card);
...
function createOfficeZone(projectName, zoneIndex, instanceCount) {
  ...
  const floor = new THREE.Mesh(floorGeo, floorMat);
  floor.rotation.x = -Math.PI / 2;
  floor.position.set(zoneX, 0.02, zoneZ);
  floor.receiveShadow = true;
  scene.add(floor);
...
async function fetchClaudeInstances() {
  try {
    const response = await fetch(`${API_URL}/api/claude-instances`);
...
function animate() {
  ...
  for (const avatar of avatars.values()) {
    if (avatar.model.visible) {
      ...
      // Switch animations based on status
      // RobotExpressive has: Idle, Walking, Running, Dance, Death, Sitting, Standing,
      // Jump, Yes, No, Wave, Punch, ThumbsUp
      let targetAction;
      if (avatar.status === 'active') {
        targetAction = 'Wave'; // Use Wave for typing motion (hands moving)
      } else if (avatar.status === 'idle') {
        targetAction = 'Idle'; // Slow idle for sleeping
      } else {
        targetAction = 'Idle';
      }

      if (avatar.actions && avatar.currentAction !== targetAction) {
        // Cross-fade to new animation
        const prevAction = avatar.actions[avatar.currentAction];
        const nextAction = avatar.actions[targetAction] || avatar.actions['Idle'];
        ...
        if (nextAction) {
          ...
          // Slow down animation for sleeping
          if (avatar.status === 'idle') {
            nextAction.timeScale = 0.3;
          } else {
            nextAction.timeScale = 1.0;
          }
          avatar.currentAction = targetAction;
        }
      }
      ...
      if (ws) {
        ...
        if (avatar.status === 'active' && isActuallyWorking) {
          ...
          // Vary animation based on mode
          if (avatar.actions) {
            let targetAnim = 'Wave'; // Default typing
            if (mode === 'thinking') {
              targetAnim = 'Idle';
            } else if (mode === 'reading') {
              targetAnim = 'Wave';
            ...
            if (avatar.currentAction !== targetAnim && avatar.actions[targetAnim]) {
              ...
              if (nextAction) {
                nextAction.reset().fadeIn(0.5).play();
                nextAction.timeScale = mode === 'thinking' ? 0.3 : 1.0;
              }
          ...
          if (isCoffeeDrinking) {
            ...
            // Walk in a circle/oval around their workstation
            const centerX = ws.position.x;
            const centerZ = ws.position.z + 1.0;
            const radiusX = avatar.coffeeBreakState.walkRadius;
            const radiusZ = avatar.coffeeBreakState.walkRadius * 0.7;

            const walkX = centerX + Math.sin(avatar.coffeeBreakState.walkAngle) * radiusX;
            const walkZ = centerZ + Math.cos(avatar.coffeeBreakState.walkAngle) * radiusZ;

            avatar.model.position.set(walkX, 0, walkZ);

            // Face the direction of movement
            const facingAngle = avatar.coffeeBreakState.walkAngle + Math.PI / 2;
...
async function fetchUsageStats() {
  try {
    const response = await fetch('http://localhost:8790/api/usage');
    const data = await response.json();
