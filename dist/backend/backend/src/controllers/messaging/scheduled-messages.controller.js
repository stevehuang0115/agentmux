...
export async function createScheduledMessage(req, res) {
    try {
        ...
        if (!name || !targetTeam || !message || !delayAmount || !delayUnit) {
            res.status(400).json({ success: false, error: 'Missing required fields: name, targetTeam, message, delayAmount, and delayUnit' });
            return;
        }
        ...
        res.json({ success: true, data: scheduledMessage, message: 'Scheduled message created successfully' });
    }
    catch (error) {
...
export async function getScheduledMessage(req, res) {
    try {
        ...
        res.status(500).json({ success: false, error: 'Failed to get scheduled message' });
    }
}
export async function updateScheduledMessage(req, res) {
    try {
        ...
        res.json({ success: true, data: updatedMessage, message: 'Scheduled message updated successfully' });
    }
    catch (error) {
...
export async function toggleScheduledMessage(req, res) {
    try {
        ...
        if (updatedMessage.isActive)
            ...
            this.messageSchedulerService?.cancelMessage(id);
        res.json({ success: true, data: updatedMessage, message: `Scheduled message ${updatedMessage.isActive ? 'activated' : 'deactivated'}` });
    }
    catch (error) {
        console.error('Error toggling scheduled message:', error);
        res.status(500).json({ success: false, error: 'Failed to toggle scheduled message' });
    }
}
export async function runScheduledMessage(req, res) {
    try {
        ...
        let error;
        try {
            const sessionName = scheduledMessage.targetTeam === 'orchestrator' ? AGENTMUX_CONSTANTS.SESSIONS.ORCHESTRATOR_NAME : scheduledMessage.targetTeam;
        ...
        await this.storageService.saveScheduledMessage(updatedMessage);
        res.json({ success: true, data: { delivered: success, deliveryLog }, message: success ? 'Scheduled message sent successfully' : `Failed to send message: ${error}` });
    }
    catch (error) {
        console.error('Error running scheduled message:', error);
        res.status(500).json({ success: false, error: 'Failed to run scheduled message' });
    }
}
