...
export class TmuxCommandService {
    ...
    shadowClients = new Set(); // Sessions with shadow clients
    shadowClientProcesses = new Map(); // Track PIDs for cleanup
    shadowClientLocks = new Map(); // Prevent race conditions
    focusEventsEnabled = false; // Track if we've enabled focus events
    constructor() {
     ...
     */
    async enableFocusEvents() {
        if (this.focusEventsEnabled)
            return;
        try {
     ...
     */
    async findSessionPaneId(sessionName) {
        try {
     ...
     */
    async ensureShadowClient(sessionName) {
        // Check if we already have a shadow client or one is being created
        if (this.shadowClients.has(sessionName))
            return;
     ...
     */
    async ensureShadowClientInternal(sessionName) {
        try {
            ...
            shadowProcess.on('error', (error) => {
                ...
                throw new Error(`Failed to start shadow client for ${sessionName}: ${error.message}`);
            });
     ...
     */
    cleanupShadowClient(sessionName) {
     ...
     */
    async sendControlKey(sessionName, key) {
        try {
