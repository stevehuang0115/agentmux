...
export class WorkflowService {
     ...
     */
    createProjectStartSteps() {
        return [
            {
                id: 'check_orchestrator',
                name: 'Check Orchestrator Session',
                status: 'pending',
                timestamp: new Date()
            },
            {
                id: 'create_orchestrator',
                name: 'Create Orchestrator Session',
                status: 'pending',
                timestamp: new Date()
            },
            {
                id: 'initialize_claude',
                name: 'Initialize Claude in Orchestrator',
                status: 'pending',
                timestamp: new Date()
            },
            {
     ...
     */
    async executeProjectStartWorkflow(execution, project, team) {
        try {
            ...
            // Step 1: Check orchestrator session
            await this.executeStep(execution, 'check_orchestrator', async () => {
                ...
                return {
                    success: true,
                    message: exists ? 'Orchestrator session already exists' : 'Orchestrator session needs to be created'
                };
            });
            // Step 2: Create orchestrator (if needed)
            await this.executeStep(execution, 'create_orchestrator', async () => {
                ...
                const result = await this.tmuxService.createOrchestratorSession(config);
                return {
            ...
            await this.executeStep(execution, 'initialize_claude', async () => {
                const orchestratorSession = execution.orchestratorSession;
                const result = await this.tmuxService.initializeOrchestrator(orchestratorSession, 45000); // 45 second timeout
                return {
                    success: result.success,
                    message: result.message,
                    error: result.error
            ...
            // Step 5: Send project start prompt to orchestrator
            ...
            // Step 6: Monitor setup (start monitoring but don't wait for completion)
     ...
     */
    async executeStep(execution, stepId, stepFunction) {
     ...
     */
    startTeamSetupMonitoring(execution) {
     ...
     */
    async sendTeamMemberPrompt(sessionName, project, member, team) {
     ...
     */
    buildTeamMemberPrompt(project, member, team) {
