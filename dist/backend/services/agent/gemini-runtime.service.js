...
export class GeminiRuntimeService extends RuntimeAgentService {
     ...
     */
    async initializeGeminiInSession(sessionName) {
        try {
     ...
     */
    async addProjectToAllowlist(sessionName, projectPath) {
        try {
            ...
            return {
                success: false,
                message: `Failed to add project path to allowlist: ${error instanceof Error ? error.message : String(error)}`,
            };
        }
    }
    /**
