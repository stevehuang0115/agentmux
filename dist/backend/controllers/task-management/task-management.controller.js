...
export async function assignTask(req, res) {
    try {
        ...
        await this.taskTrackingService.assignTask(project.id, teamId, targetPath, taskInfo.title || taskInfo.fileName, taskInfo.targetRole || 'unknown', memberId, sessionId || 'unknown');
        res.json({
            success: true,
            message: `Task ${fileName} assigned to member ${memberId}`,
...
export async function startTaskExecution(req, res) {
    try {
        const { taskPath, projectPath, projectName, taskId, taskTitle, taskDescription, taskPriority = 'medium', taskMilestone = 'm0_build_spec_tasks', retryCount = 3, timeoutSeconds = 30, } = req.body;
...
export async function createTasksFromConfig(req, res) {
    try {
        ...
        try {
            ...
            console.warn(`Failed to read initial goal file: ${error instanceof Error ? error.message : error}`);
        }
        try {
