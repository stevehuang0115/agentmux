...
class AgentMuxMCP {
    ...
    projectPath;
    agentRole;
    requestQueue = new Map(); // Rate limiting
    ...
    registerTools() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                ...
                case 'accept_task':
                    return await this.acceptTask(args);
                case 'complete_task':
                    return await this.completeTask(args);
                case 'block_task':
                    return await this.blockTask(args);
                case 'take_next_task':
                    return await this.takeNextTask(args);
                case 'sync_task_status':
                    return await this.syncTaskStatus(args);
                case 'check_team_progress':
                    return await this.checkTeamProgress(args);
                case 'read_task_file':
                    return await this.readTaskFile(args);
                case 'report_ready':
                    return await this.reportReady(args);
        ...
        try {
            ...
            for (let i = 0; i < sessions.length; i += maxConcurrent) {
                ...
                const batchPromises = batch.map(async (session) => {
                    if (args.excludeSelf && session === this.sessionName)
                        return;
                    try {
            ...
            try {
                ...
                for (const file of files.stdout.split('\n')) {
                    ...
                    try {
                        ...
                        if (!args.all && ticket.assignedTo !== this.sessionName)
                            continue;
                ...
                for (const statusFolder of statusFolders) {
                    ...
                    try {
                        ...
                        for (const taskFile of taskFiles) {
                            ...
                            // Filter by assignment and status
                            if (!args.all && statusFolder !== 'in_progress')
                                continue;
                            tickets.push(task);
                        }
                    }
...
Context not yet loaded. Use load_project_context to initialize.`;
    ...
    async refreshAgentContext() {
        try {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `Context refreshed successfully. Updated at: ${result.data?.refreshedAt || new Date().toISOString()}`
                    }],
            };
        }
        catch (error) {
    ...
    extractContextSection(content, sectionName) {
            ...
            if (result.data?.teamMembers) {
                for (const member of result.data.teamMembers) {
                    ...
                    if (member.currentTasks?.length > 0) {
                        progressReport += `- Tasks: ${member.currentTasks.map((t) => t.name).join(', ')}\n`;
                    }
...
- Ensure quality standards are maintained`
    ...
    async writeSystemPrompt(promptPath, prompt, config) {
...
Work autonomously within your role boundaries and communicate effectively with your team.`;
    ...
    async logScheduledCheck(target, minutes, message) {
