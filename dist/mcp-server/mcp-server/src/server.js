...
export class AgentMuxMCPServer {
    ...
    async broadcast(params) {
        try {
            ...
            const sessionNames = sessions.map((s) => s.sessionName);
            // Process sessions in batches
            for (let i = 0; i < sessionNames.length; i += maxConcurrent) {
                const batch = sessionNames.slice(i, i + maxConcurrent);
                const batchPromises = batch.map(async (sessionName) => {
                    if (params.excludeSelf && sessionName === this.sessionName)
                        return;
                    try {
    ...
    async getAgentStatus(params) {
        ...
        if (backendData?.success && backendData?.data?.teams) {
            ...
            for (const team of backendData.data.teams) {
                if (team.members) {
                    const member = team.members.find((m) => m.sessionName === targetSession ||
     ...
     */
    mapAgentStatus(backendStatus, sessionExists) {
        if (!backendStatus) {
     ...
     */
    async registerAgentStatus(params) {
        const startTime = Date.now();
            ...
            try {
                ...
                for (const file of files) {
                    if (file.endsWith('.yaml') || file.endsWith('.yml')) {
                        ...
                        if (!all && ticket.assignedTo !== this.sessionName)
                            continue;
                        ...
                        try {
                            ...
                            for (const file of statusFiles) {
                                if (file.endsWith('.md')) {
                                    ...
                                    if (!all && ticket.assignedTo !== this.sessionName)
                                        continue;
                                    if (status && ticket.status !== statusDir)
                                        continue;
                                    tickets.push(ticket);
                                }
                            }
        ...
        try {
            ...
            if (ticket.path?.endsWith('.yaml') || ticket.path?.endsWith('.yml')) {
                ...
                if (frontmatterMatch) {
                    ...
                    if (status)
                        frontmatter = this.updateYAMLField(frontmatter, 'status', status);
                    if (notes)
                        body = `${body}\n\n## Update Notes\n${notes}`;
                    if (blockers)
                        frontmatter = this.updateYAMLField(frontmatter, 'blockers', blockers);
                    updatedContent = `---\n${frontmatter}\n---\n${body}`;
                }
                else {
    ...
    async reportProgress(params) {
        try {
            ...
            const orchestrator = orchestratorSessions.find((s) => s.sessionName.includes('orchestrator') || s.sessionName.includes('orc'));
            if (orchestrator) {
    ...
    async requestReview(params) {
        try {
            ...
            if (reviewer) {
                ...
                let sentCount = 0;
                for (const sessionInfo of sessions) {
                    if (sessionInfo.sessionName === this.sessionName)
                        continue;
                    try {
    ...
    parseMDTicket(content, fileName, status, milestone) {
        const ticket = {
    ...
    updateYAMLField(frontmatter, field, value) {
        const lines = frontmatter.split('\n');
        let updated = false;
        for (let i = 0; i < lines.length; i++) {
    ...
    async logDelegation(from, to, task, priority) {
        const fs = await import('fs/promises');
        const path = await import('path');
    ...
    async logTaskDelegation(taskPath, from, to, reason, delegationChain) {
        const fs = await import('fs/promises');
        const path = await import('path');
    ...
    parseTaskContent(content, taskPath) {
...
Please respond promptly with either acceptance or delegation.`;
    ...
    async buildAssignmentMessage(params) {
        const { template, taskPath = '', taskDetails = {}, taskProjectPath = '', targetSessionName, delegatedBy = this.sessionName, reason = '', delegationChain = [], task = '', priority = 'normal', ticketId = '' } = params;
        const path = await import('path');
        // Build delegation chain
    ...
    analyzeAgentStatus(output) {
        ...
        if (activeGitPatterns.some(pattern => output.match(pattern)))
            return 'committing';
        // Check for waiting patterns
        if (output.includes('waiting') || output.includes('Waiting'))
            return 'waiting';
        if (output.includes('STATUS UPDATE'))
            return 'reporting';
        // Default to working only if none of the above patterns match
        return 'working';
    }
    extractLastActivity(output) {
        const lines = output.split('\n').filter(l => l.trim());
        return lines[lines.length - 1] || 'No recent activity';
    }
    getDefaultNameFromSession(sessionName) {
        // Handle orchestrator
     ...
     */
    async addTaskToInProgressTracking(taskPath, sessionName, assignmentResult) {
        try {
            const fs = await import('fs/promises');
            const path = await import('path');
            ...
            // Load existing data
            let trackingData = { tasks: [], lastUpdated: new Date().toISOString(), version: '1.0.0' };
            try {
     ...
     */
    async removeTaskFromInProgressTracking(taskPath) {
        try {
            const fs = await import('fs/promises');
            const path = await import('path');
            const os = await import('os');
            ...
            trackingData.tasks = trackingData.tasks.filter((task) => task.taskPath !== taskPath &&
