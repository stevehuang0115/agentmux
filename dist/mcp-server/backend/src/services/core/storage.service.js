        ...
        try {
            ...
            if (team.members) {
                team.members.forEach(member => {
                    console.log(`[STORAGE-DEBUG-TS] 👤 Saving member ${member.name}: agentStatus=${member.agentStatus}, workingStatus=${member.workingStatus}, readyAt=${member.readyAt}`);
                });
            }
            await this.ensureFile(this.teamsFile, { teams: [], orchestrator: this.createDefaultOrchestrator() });
            const content = await fs.readFile(this.teamsFile, 'utf-8');
...
`;
    ...
    async updateOrchestratorStatus(status) {
        return this.updateAgentStatus(CONFIG_CONSTANTS.SESSIONS.ORCHESTRATOR_NAME, status);
    }
}
