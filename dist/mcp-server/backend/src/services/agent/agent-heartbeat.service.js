...
import { AGENTMUX_CONSTANTS, AGENT_IDENTITY_CONSTANTS, TIMING_CONSTANTS } from '../../constants.js';
/**
...
export class AgentStatusBatcher {
    ...
    flushTimer = null;
    maxBatchSize = 50;
    flushTimeoutMs = TIMING_CONSTANTS.TIMEOUTS.TASK_MONITOR_POLL; // 2 seconds
    heartbeatService;
     ...
     */
    addUpdate(agentId, sessionName, agentStatus, teamMemberId) {
...
export class AgentHeartbeatService {
     ...
     */
    static getInstance(agentmuxHome) {
     ...
     * ```
     */
    async updateAgentHeartbeat(sessionName, teamMemberId, agentStatus = AGENTMUX_CONSTANTS.AGENT_STATUSES.ACTIVE) {
        try {
     ...
     */
    determineAgentId(sessionName, teamMemberId) {
     ...
     */
    applyHeartbeatUpdate(statusData, update) {
        ...
        if (update.agentId === AGENT_IDENTITY_CONSTANTS.ORCHESTRATOR.ID) {
            ...
            if (!statusData.teamMembers[memberId]) {
                statusData.teamMembers[memberId] = this.createDefaultTeamMemberHeartbeat(update.agentId, update.sessionName, memberId);
            }
     ...
     */
    async saveTeamAgentStatusFile(data) {
     ...
     */
    async ensureTeamAgentStatusFile() {
     ...
     */
    createDefaultTeamAgentStatusFile() {
        return {
     ...
     */
    createDefaultOrchestratorHeartbeat() {
        const now = new Date().toISOString();
        return {
     ...
     */
    createDefaultTeamMemberHeartbeat(agentId, sessionName, teamMemberId) {
        const now = new Date().toISOString();
        return {
            agentId,
            sessionName,
     ...
     */
    async detectStaleAgents(thresholdMinutes = 30) {
        try {
            const statusData = await this.loadTeamAgentStatusFile();
            const staleAgents = [];
 ...
 * ```
 */
export async function updateAgentHeartbeat(sessionName, teamMemberId, agentStatus = AGENTMUX_CONSTANTS.AGENT_STATUSES.ACTIVE) {
    const service = AgentHeartbeatService.getInstance();
    await service.updateAgentHeartbeat(sessionName, teamMemberId, agentStatus);
}
//# sourceMappingURL=agent-heartbeat.service.js.map