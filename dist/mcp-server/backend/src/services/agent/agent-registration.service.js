...
import { AGENTMUX_CONSTANTS, ENV_CONSTANTS, AGENT_TIMEOUTS, ORCHESTRATOR_ROLE, RUNTIME_TYPES, } from '../../constants.js';
/**
...
export class AgentRegistrationService {
     ...
     */
    async initializeAgentWithRegistration(sessionName, role, projectPath, timeout = AGENT_TIMEOUTS.REGULAR_AGENT_INITIALIZATION, memberId, runtimeType = RUNTIME_TYPES.CLAUDE_CODE) {
        ...
        try {
            ...
            projectPath, memberId, runtimeType);
            if (step1Success) {
                return {
                    success: true,
     ...
     */
    async tryCleanupAndReinit(sessionName, role, timeout, projectPath, memberId, runtimeType = RUNTIME_TYPES.CLAUDE_CODE) {
        // Clear Commandline
        ...
        return await this.attemptRegistrationWithVerification(sessionName, role, timeout, memberId, 3, true, runtimeType);
    }
    /**
     * Step 3: Kill session and recreate completely
     * Most aggressive approach - completely recreates the session from scratch
     */
    async tryFullRecreation(sessionName, role, timeout, projectPath, memberId, runtimeType = RUNTIME_TYPES.CLAUDE_CODE) {
        ...
        if (role === ORCHESTRATOR_ROLE) {
            ...
            this.logger.debug('Runtime confirmed ready for orchestrator in Step 3, sending registration prompt', { sessionName, runtimeType });
            // Send Ctrl+C to cancel any pending slash command from detection
            await this.tmuxCommand.sendCtrlC(sessionName);
        }
        else {
            ...
            // Use shorter check interval in test environment
            const checkInterval = process.env.NODE_ENV === 'test' ? 1000 : 2000;
            const isReady = await RuntimeServiceFactory.create(runtimeType, this.tmuxCommand, this.projectRoot).waitForRuntimeReady(sessionName, 25000, checkInterval); // 25s timeout
            if (!isReady) {
    ...
    async createAgentSession(config) {
        ...
        try {
            ...
            await this.tmuxCommand.setEnvironmentVariable(sessionName, ENV_CONSTANTS.AGENTMUX_ROLE, role);
            ...
            const initResult = await this.initializeAgentWithRegistration(sessionName, role, projectPath, timeout, memberId, runtimeType);
    ...
    async terminateAgentSession(sessionName, role = 'unknown') {
        try {
            ...
            await this.storageService.updateAgentStatus(sessionName, AGENTMUX_CONSTANTS.AGENT_STATUSES.INACTIVE);
     ...
     */
    async sendPromptRobustly(sessionName, prompt) {
        const maxAttempts = 3;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                const hasProcessingIndicators = /thinking|processing|analyzing|registering/i.test(afterOutput);
                if (lengthIncrease > 50 || hasProcessingIndicators || !hasPromptInOutput) {
                    ...
                    this.logger.debug('Prompt delivery verified - Claude appears to be processing', {
                        sessionName,
                        attempt,
                        lengthIncrease,
