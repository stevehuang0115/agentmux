...
export async function trackError(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!message) { res.status(400).json({ success: false, error: 'Error message is required' } as ApiResponse); return; }
    const errorTracker = ErrorTrackingService.getInstance();
    const errorId = errorTracker.trackError(message, { level: level || 'error', source: source || 'frontend', component, action, metadata, sessionId: req.headers['x-session-id'] as string, userId: req.headers['x-user-id'] as string });
    res.status(201).json({ success: true, data: { errorId }, message: 'Error tracked successfully' } as ApiResponse);
  } catch (error) {
...
export async function getErrorStats(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to get error statistics' } as ApiResponse);
  }
}
...
export async function getError(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!errorEvent) { res.status(404).json({ success: false, error: 'Error not found' } as ApiResponse); return; }
...
export async function clearErrors(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    ...
    res.json({ success: true, data: { removedCount }, message: `Cleared ${removedCount} error records` } as ApiResponse);
  } catch (error) {
