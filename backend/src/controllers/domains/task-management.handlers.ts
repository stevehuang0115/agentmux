...
export async function startTaskExecution(
	...
	try {
		...
		const promptTemplate = await readFile(
			join(process.cwd(), 'config', 'prompts', 'assign-task-orchestrator-prompt-template.md'),
			'utf-8'
		);
...
function generateTaskMarkdown(step: any, projectVars: any): string {
...
function addTaskAssignmentInfo(content: string, memberId: string, sessionId?: string): string {
