...
import { ContextLoaderService, WorkflowService, TicketEditorService, TaskService } from '../../services/index.js';
...
export async function createProject(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      ...
      const isOrchestratorActive = orchestratorStatus?.agentStatus === AGENTMUX_CONSTANTS.AGENT_STATUSES.ACTIVE;
    ...
    res.status(201).json({ success: true, data: finalProject, message: 'Project added successfully' } as ApiResponse<Project>);
  } catch (error) {
 ...
 */
export async function getAlignmentStatus(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params as any;
    const projects = await this.storageService.getProjects();
    const project = projects.find(p => p.id === id);
    ...
    res.status(500).json({ success: false, error: 'Failed to get alignment status' } as ApiResponse);
  }
}
 ...
 */
export async function continueWithMisalignment(this: ApiContext, req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params as any;
    const projects = await this.storageService.getProjects();
    const project = projects.find(p => p.id === id);
    ...
    res.json({ success: true, message: 'Continuing with misalignment acknowledged' } as ApiResponse);
  } catch (error) {
    console.error('Error handling continue with misalignment:', error);
    res.status(500).json({ success: false, error: 'Failed to handle continue with misalignment' } as ApiResponse);
  }
}
