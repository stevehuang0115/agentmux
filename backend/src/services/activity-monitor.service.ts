...
export class ActivityMonitorService {
  ...
  private async updateOrchestratorWithStatuses(status: string, agentStatus: string, workingStatus: string): Promise<void> {
    try {
      const content = JSON.parse(await require('fs/promises').readFile(
        require('path').join(require('os').homedir(), '.agentmux', 'teams.json'), 'utf-8'
      ));
      ...
      await require('fs/promises').writeFile(
        require('path').join(require('os').homedir(), '.agentmux', 'teams.json'),
