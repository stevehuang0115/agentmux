...
export class FileWatcherService extends EventEmitter {
  ...
  private projectWatchers: Map<string, Set<string>> = new Map(); // projectId -> Set of watched paths
  private eventCounts: Map<string, number> = new Map();
  private debounceTimers: Map<string, NodeJS.Timeout> = new Map();
  private lastEvents: Map<string, Date> = new Map();
   ...
   */
  async stopWatchingProject(projectId: string): Promise<void> {
    const watcherId = `project_${projectId}`;
  ...
  private createWatcher(
    dirPath: string, 
    projectId: string, 
    category: 'main' | 'specs' | 'tasks' | 'memory' | 'prompts'
  ): fs.FSWatcher {
    const watcher = fs.watch(dirPath, { recursive: true }, (eventType, filename) => {
      ...
      this.handleFileChange(eventType as 'change' | 'rename', filepath, relativePath, projectId, category);
    });
  ...
  private async processFileChange(
    ...
    const changeType: FileChangeEvent['type'] = this.determineChangeType(eventType, fileExists, filepath);
   ...
   */
  private categorizeFile(relativePath: string): FileChangeEvent['category'] {
   ...
   * Trigger specific actions based on file type
   */
  private async triggerFileSpecificActions(event: FileChangeEvent): Promise<void> {
    try {
   ...
   */
  private updateStats(projectId: string, event: FileChangeEvent): void {
   ...
   */
  async watchAllProjects(): Promise<void> {
    try {
