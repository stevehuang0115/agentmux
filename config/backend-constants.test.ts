...
import {
  ...
  // Detection and performance constants
  CLAUDE_DETECTION_CACHE_TIMEOUT,
  ...
  describe('Role Constants', () => {
    ...
    test('should have display names for all roles', () => {
      Object.values(AGENT_ROLES).forEach((role) => {
        ...
        expect(typeof ROLE_DISPLAY_NAMES[role as keyof typeof ROLE_DISPLAY_NAMES]).toBe('string');
      });
    });
  ...
  describe('Constants Immutability', () => {
    test('object constants should be frozen (const assertions)', () => {
      ...
      expect(() => {
        ...
        (AGENT_ROLES as any).ORCHESTRATOR = 'changed';
      }).toThrow();
    });
  });
