# Project Goals


### [2026-02-21T16:10:07.714Z] Set by orchestrator
把Crewly做到像OpenClaw一样有影响力 — 研究OpenClaw功能/社区/生态，制定升级计划、GTM策略和资源规划

### [2026-02-21T21:39:59.824Z] Set by orchestrator
Crewly Core 全自动运营团队 OKR（分阶段）

## Phase 1: 调研 + 研发（现在启动）
团队：Mia + Sam

O1 [调研]: 摸清差距，制定路线 (Mia)
- KR1: 完成 Crewly vs OpenClaw/CrewAI/AutoGen 功能差距矩阵
- KR2: 输出优先级排序 Roadmap
- KR3: 每两周更新竞品动态，Roadmap 随之迭代

O2 [研发]: 快速迭代，关闭差距 (Sam)
- KR1: 按 Roadmap 优先级，每天出 feature
- KR2: 代码质量底线（测试80%+，TS strict，关键路径文档）
- KR3: 开源发布准备（LICENSE, CONTRIBUTING, README, npx crewly init）

Phase 1→2 触发：3+ 差异化核心功能 + 完整 demo flow + 用户确认

## Phase 2: 加入推广（产品就绪后启动）
团队：+1 Marketing/Creative

O3 [推广]: 建立市场存在感 (新成员)
- KR1: 每周1篇技术博客/教程（Nano Banana 配图）
- KR2: 制作 Demo 视频 + 宣传素材（Veo/Flopost）
- KR3: 提交 HN / Product Hunt / 开发者目录
- KR4: 3 个预设团队模板作为 onboarding 钩子

Phase 2→3 触发：首次公开发布 + 有外部用户 + 用户确认

## Phase 3: 加入运营 + 支持（有用户后启动）
团队：+1~2 Community/Ops + Support

O4 [运营]: 社区活跃与开发者关系 (新成员)
- KR1: Discord 每日活跃
- KR2: Reddit/X/HN 社区互动
- KR3: 每周 newsletter

O5 [支持]: 用户响应与反馈闭环 (新成员/Mia兼)
- KR1: email/issue 24h 内响应
- KR2: FAQ + troubleshooting 文档
- KR3: Bug→Sam，反馈→Mia 影响 Roadmap

工具链：图片(Nano Banana✅) 视频(Veo/Flopost🔧) 社媒/邮件(API✅) UI(Stitch✅)
