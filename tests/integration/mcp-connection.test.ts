...
describe('MCP Server Integration Tests', () => {
  ...
  describe('MCP Protocol', () => {
    ...
    it('should list available tools', async () => {
      ...
      const registerTool = body.result.tools.find((tool: any) => tool.name === 'register_agent_status');
      expect(registerTool).toBeDefined();
      expect(registerTool.description).toBe('Register agent as active and ready to receive instructions');
  ...
  describe('Tool Execution', () => {
    ...
    it('should execute register_agent_status tool with sessionId', async () => {
      ...
      expect(body.result.content[0].text).toMatch(/(registered successfully|Failed to register agent)/);
    ...
    it('should handle send_message tool with rate limiting', async () => {
      ...
      expect(body2.result.content[0].text).toMatch(/(Rate limit exceeded|Failed to send message)/);
    });
